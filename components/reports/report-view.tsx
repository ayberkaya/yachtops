...
export function ReportView({ data, onBack }: ReportViewProps) {
  ...
  data.expensesByMonth.forEach((item) => {
    const monthKey = format(parseISO(`${item.month}-01`), "MMM yyyy", { locale: enUS });
  ...
  data.expensesByMonth.forEach((item) => {
    const formatted = format(parseISO(`${item.month}-01`), "MMM yyyy", { locale: enUS });
          ...
          <div className="flex items-start justify-between">
            <div>
              <CardTitle className="text-2xl print:text-xl">
                {data.yacht.name} - Financial Report
              </CardTitle>
              <CardDescription className="mt-2 print:text-sm">
                Report Date: {format(parseISO(data.generatedAt), "dd MMMM yyyy, HH:mm", {
                  locale: enUS,
                })}
                <br />
                ...
                {data.expenses.length > 0 &&
                  `${format(parseISO(data.expenses[data.expenses.length - 1].date), "dd MMM yyyy", {
                    locale: enUS,
                  })} - ${format(parseISO(data.expenses[0].date), "dd MMM yyyy", { locale: enUS })}`}
              </CardDescription>
            </div>
            {data.yacht.logoUrl && (
              <img
                                ...
                                {currencyExpenses.map((expense, index) => (
                                  ...
                                  >
                                    <td className="p-2 print:p-[6px] print:text-xs text-sm">
                                      {format(parseISO(expense.date), "dd MMM yyyy", { locale: enUS })}
                                    </td>
