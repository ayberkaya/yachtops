...
export async function acceptInvite(
  ...
  try {
    ...
    if (existingUser) {
      // User already exists - mark invitation as accepted to clean up the pending state
      // This handles the case where user was created but invitation wasn't marked as accepted
      await db.yachtInvite.update({
        where: { id: invite.id },
    ...
    // Use transaction to atomically create user and mark invitation as accepted
